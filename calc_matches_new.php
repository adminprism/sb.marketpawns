...
define("LVL_LOST", 80); // потеря уровня МП
...
--  `m1_NN1` float DEFAULT NULL,
--  `m1_NN2` float DEFAULT NULL,
--  `m1_NN3` float DEFAULT NULL,
  `Koef` float NOT NULL,
--  `m2_NN1` float DEFAULT NULL,
--  `m2_NN2` float DEFAULT NULL,
--  `m2_NN3` float DEFAULT NULL,
  KEY `model` (`model_id`),
  KEY `link` (`link_id`)
...
if ($isALL) {
    ...
    //$result = queryMysql("update models set Clst_I=null,Clst_II=null,Clst_A=null,Clst_I_E=null,Clst_II_E=null,Clst_A_E=null;");
} else {
    ...
    //$result = queryMysql("update models set Clst_I=null,Clst_II=null,Clst_A=null,Clst_I_E=null,Clst_II_E=null,Clst_A_E=null where  name_id in (".$ourTools[$tool]['name_id_list'].");");
}
...
foreach ($ourTools as $cur_tool => $toolInfo) if ($isALL || $cur_tool == $tool) { // перебор всех инструментов (выполнится один раз, если задан конкретный)
    ...
    foreach ($ClstTable as $model_id => $clstArr) {
        ...
        write_log("setList : ", $setList . PHP_EOL, 9);
                    ...
                    for ($j = $j_m; $j < $model1_cnt; $j++) { // перебор кандидатов в Модель1 (Для спаривания с текущим кандидатом в Модель2)
                        ...
                        foreach ($aimList as $pk1 => $aimName1) if (isset($AimsAndSizes1[$j][$aimName1])) { // перебираем все наименования цели кондидата в Модель1
                            ...
                            if (abs($node1_level - $node2_level) < $node2_size_level * YI_PROC / 100) { //3. Расстояние между уровнями моделей 1 и 2 находится в пределах YI, где YI - величина в % от размера Модели 2 (по умолчанию = 3%).
                                ...
                                if (($ContextClst[$id2][$clstInd] ?? 0) < $node1_size_level / $node2_size_level) $ContextClst[$id2][$clstInd] = $node1_size_level / $node2_size_level;
                            } // по уровням ОК (вар.2)
